...
type LatestActionRow = Prisma.AgentActionGetPayload<{
...
async function getDashboardData(page: number = 1) {
  ...
  const rawEscalations = await prisma.agentAction.findMany({
    where: {
      actionType: {
        in: ["ESCALATE_COMPLIANCE", "ESCALATE_MANAGEMENT", "ALERT_ADVISOR_STUCK"],
      },
    },
    orderBy: { performedAt: "desc" },
    take: 15,
  ...
  const initialActions = latestActions.map((a: LatestActionRow) => ({
    ...a,
  ...
  const vaults = clients.map((client: DashboardClient) => {
    let expiredCount = 0;
    ...
    let highestUrgency: "CRITICAL" | "HIGH" | "MEDIUM" | "LOW" | "NONE" = "NONE";
  ...
  vaults.sort(
    (a, b) =>
      (rank[b.urgency.highest as keyof typeof rank] ?? 0) -
      (rank[a.urgency.highest as keyof typeof rank] ?? 0)
  );

  const summary = {
    totalClients: totalCount,
    criticalClients: vaults.filter((v) => v.urgency.highest === "CRITICAL")
      .length,
    ...
    <div className="space-y-6 max-w-[1600px] mx-auto pb-10">
      <div>
        ...
        <p className="text-muted-foreground">
          Monitor and manage all client vaults with autonomous agents.
        </p>
      </div>
